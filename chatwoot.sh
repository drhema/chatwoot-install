...
function upgrade_redis() {
    ...
    current_version=$(redis-server --version | grep -oP '(?<=v=)\d+\.\d+\.\d+')
    ...
    # Ensure the GPG key is imported for Redis package verification
    curl -fsSL https://packages.redis.io/gpg | sudo gpg --dearmor -o /usr/share/keyrings/redis-archive-keyring.gpg

    # Add the Redis repository
    echo "deb [signed-by=/usr/share/keyrings/redis-archive-keyring.gpg] https://packages.redis.io/deb $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/redis.list

    # Handle potential 403 Forbidden error by checking for repository availability
    if ! sudo apt-get update; then
        echo "Error updating package lists. Please check your repository configuration."
        return
    fi

    # Upgrade Redis server to the latest version
    if ! sudo apt-get install --only-upgrade redis-server -y; then
        echo "Error upgrading Redis server. Please check for available Redis versions and compatibility."
        return
    fi

    # Install libvips for image processing, required by Chatwoot
    if ! sudo apt-get install libvips -y; then
        echo "Error installing libvips. Please check for package availability."
        return
    fi
